    ...
    if (store.getters['routes/routes'].length) {
      ...
      if (to.path === '/') {
        if (store.state.user.userType == 1) {
          next({ path: '/project/indexManagement' })
        } else if (store.state.user.userType == 2)
          if (showProgressBar) {
            next({ path: 'majorIndex' })
          }
        // if (JSON.parse(localStorage.getItem('userInfo').state) == 1) {
        //   next({ path: '/project/indexManagement' })
        // } else if (JSON.parse(localStorage.getItem('userInfo').state) == 2) {
        //   next({ path: '/majorIndex' })
        // }
        VabProgress.done()
      } else next()
